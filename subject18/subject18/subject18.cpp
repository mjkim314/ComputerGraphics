...
void render(GLFWwindow* window) {
	...
	radian += 0.01;
	glm::vec3 roaxis = glm::vec3(0.0, 1.0, 0.0);
	glm::mat4 roMat = glm::rotate(radian, roaxis);

	glUseProgram(program);
	GLuint loc = glGetUniformLocation(program, "modelMat");
	glUniformMatrix4fv(loc, 1, 0, glm::value_ptr(roMat));
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexVBOID);
